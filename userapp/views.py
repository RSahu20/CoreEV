...
def registerConsumerSocial(request):
    ...
    context = {"consumerform": consumerform}
...
def registerConsumer(request):
    ...
    if request.method == "POST":
        ...
        consumerform = ConsumerSignUpForm()
    context = {"signupform": signupform, "consumerform": consumerform}
    return render(request, "userapp/registerConsumer.html", context=context)
...
def registerProvider(request):
    ...
    context = {"signupform": signupform, "providerform": providerform}
...
def UpdateProfile(request):
    ...
    if request.method == "GET":
        ...
        if request.user.is_consumer:
            ...
            fieldform = ConsumerSignUpForm(request.POST, instance=request.user.consumer)
        else:
            userform = UserUpdateForm(request.POST, instance=request.user)
...
def ProviderDashboard(request):
    ...
    if request.user.provider:
        if request.method == "GET":
            ...
            paginator = Paginator(
                ChargingStation.objects.filter(owner=current_provider), 3
            )
            try:
            ...
            supportform = SupportForm()
            context = {"cslist": cslist, "supportform": supportform}
            if request.method == "POST":
...
class ChargingStationProviderDeleteView(
...
def ChargingStationConsumer(request):
    ...
    context = {"csdata": json.dumps(csdata), "cslist": cslist, "pkid": json.dumps(pkid)}
...
def ChargingStationDashboard(request, pk):
    if request.user.is_provider:
        ...
        recorddata = [
            list(x)
            for x in zip(
                username_cleaned, vehicle_cleaned, duration_cleaned, consumption_cleaned
            )
        ]
        supportform = SupportForm()
        context = {
...
def ChargePooling(request):
    ...
    context = {"chargepoolers": chargepoolers, "consumer": consumer}
...
def MaintenanceComplaint(request):
    ...
    js_data = [
        list(x) for x in zip(lats_cleaned, lngs_cleaned, pk_cleaned, name_cleaned)
    ]
    ...
    return render(
        request,
        "userapp/complaint_dashboard.html",
        {
            "d": d,
            "count": count,
            "m": m,
            "total": total,
            "my_data": json.dumps(js_data),
            "supportform": supportform,
        },
    )
...
def SupportRequest(request):
    if request.method == "POST" and request.is_ajax():
        ...
        support = {"subject": subject, "description": description}
    ...
    if request.user.is_provider:
        ...
        for cs in all_cs:
            ...
            for record in all_record_cs:
                total_visits += 1
                consumption_cs += float(record.vehicle.charging_rate) * (
                    record.duration / 60
                )
